...
export const UseMemoDemo1 = () => {
  ...
  const [msg, setMsg] = useState('I LV U');

  const memoWord = useMemo(() => {
    console.log('memoWord run');
    return msg + ' ' + name; // 可以看到没有添加 name 作为依赖的时候这个 memoWord 是不改变的
  }, [msg]);
  // 这里 React 也在提示你是否缺少了 name 这个依赖项，
  // 如果 name 不是依赖项，没有必要在 useMemo 里拼接它
  // 生产使用时, 务必将依赖项补全, 否则可能导致闭包陷阱!!
  ...
  return (
    <div>
      ...
      <div>
        ...
        <input name="msg" type="text" onChange={e => setMsg(e.target.value)} value={msg} />
      </div>
      <p>Computed memo: {memoWord} </p>
      <p>↑ 当前 memo, 记录 msg 依赖, 但不记录 name 依赖</p>
    </div>
  );
};
...
const UseMemoDemo2 = () => {
  ...
  return (
    <div>
      ...
      { computing && <h4 style={{ color: 'red' }}>Complex Calculating...</h4> }
      {complexCompMemo}
      <h3>
        Count: {count} <button onClick={addCount}>+</button>
      </h3>
      <h4>Count 为 5 的倍数时,更新上面的计算组件</h4>
    </div>
  );
};
...
const UseMemoDemo3 = () => {
  const [count, setCount] = useState(0);
  const { computing, workRes } = useComplexCalcCompWorker({ x: count, y: CONST_Y })
  ...
  return (
    <div>
      <h2>
        Demo3. 使用 worker 优化性能
      </h2>
      <ComplexCalcCompWorker computing={computing} workRes={workRes} />
      <h3>
        Count: {count}
        {
          computing
          ? <span style={{ fontSize: '12px', color: 'orange', paddingLeft: '5px' }}>
              Loading...
            </span>
...
const useComplexCalcCompWorker = ({ x, y }) => {
  const [computing, setComputing] = useState(false);
  const [worker, setWorker] = useState(null);
  const [workRes, setWorkRes] = useState(null);
...
const ComplexCalcCompWorker = ({ computing, workRes }) => {
  return (
    <div style={{ border: '1px solid', padding: '30px' }}>
      <h3>ComplexCalcCompWorker 计算组件</h3>
      <div>result:
        {
          computing 
          ? <span style={{ color: 'orange' }}>Complex Calculating...</span>
          : (workRes === null ? 'waiting...' : workRes)
        }
      </div>
    </div>
  );
