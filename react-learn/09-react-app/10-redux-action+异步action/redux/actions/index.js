...
// 同步 Action 返回一般对象
export const INCREASE_ACTION = (payload) => ({ type: INCREASE, payload });

export const DECREASE_ACTION = (payload) => ({ type: DECREASE, payload });

export const MULTIPLY_ACTION = (payload) => ({ type: MULTIPLY, payload });

// 使用 redux-thunk 实现异步 Action
// 当 store.dispatch 接收的返回值是一个函数时
// 第一个传入的回调实参 dispatch === store.dispatch
export const ASYNC_INC_ACTION = (payload, wait) => {
  return (dispatch) => {
    setTimeout(() => {
      dispatch(INCREASE_ACTION(payload));
    }, wait);
  };
};
